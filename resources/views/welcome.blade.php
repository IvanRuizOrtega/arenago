...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <!-- Main Container -->
    <div class="min-h-screen bg-slate-950 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-black text-slate-200">
    ...
    <main class="relative z-10 max-w-7xl mx-auto px-4 pb-20">
        ...
        @if(session(\Src\Resources\Constants\Messages::SESSION_ERROR))
            @php
                $error = session(\Src\Resources\Constants\Messages::SESSION_ERROR);
            @endphp
            <x-modal
                :title="$error['title'] ?? null"
                :description="$error['description'] ?? null"
                :forget="$error['forget'] ?? false"
                :alertColor="$error['alertColor'] ?? null"
            />
        @endif
