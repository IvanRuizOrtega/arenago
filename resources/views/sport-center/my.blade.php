        ...
        <header class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 px-2">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <h2 class="text-3xl font-bold text-white tracking-tight">{{ \Src\Resources\Constants\Headers::getKeys()[1]??'' }}</h2>
                </div>
            </div>

            <!-- Botón de Redirección a la Derecha -->
            <a href="{{ route(\Src\Resources\Constants\Routes::SPORT_CENTER_CREATE_FORM) }}" 
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-white/5">
                            ...
                            <th class="p-6 text-[10px] font-bold uppercase text-slate-500 tracking-widest border-b border-white/5">Direccion</th>
                            <th class="p-6 text-[10px] font-bold uppercase text-slate-500 tracking-widest border-b border-white/5">Ubicacion</th>
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/[0.03] transition-colors group">
                            ...
                            <td class="p-6">
                                <span class="text-sm font-mono text-emerald-400 font-bold">{{ $item->city }}</span>
                            </td>
                            <td class="p-6 text-xs text-slate-500 font-medium">
                                {{ $item->is_open ? 'Abierto ahora' : 'Cerrado ahora'}}
                            </td>
                            <td class="p-6">
                                <div class="flex justify-end gap-3">
                                    <!-- Ver -->
                                    <a href="{{ route(\Src\Resources\Constants\Routes::MY_SPORT_CENTER_SHOW, $item->_id) }}" class="p-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-400 hover:text-white hover:bg-white/10 transition-all">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <!-- Editar -->
                                    <a href="{{ route(\Src\Resources\Constants\Routes::SPORT_CENTER_EDIT_FORM, $item->_id) }}" class="p-2.5 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-400 hover:bg-blue-600 hover:text-white transition-all">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <!-- Eliminar -->
                                    <form action="{{ route(\Src\Resources\Constants\Routes::SPORT_CENTER_FORGET, $item->_id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este elemento?');" class="inline">
                                        @csrf
                                        @method('DELETE')
